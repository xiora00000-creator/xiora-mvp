...
export default function Page({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(`/api/admin/categories/${params.slug}`)
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res.ok) r.push('/admin/categories')
  }
  
  async function onDelete() {
    if (!confirm('削除しますか？')) return
    const res = await fetch(`/api/admin/categories/${params.slug}`, { method: 'DELETE' })
    if (res.ok) r.push('/admin/categories')
  }
